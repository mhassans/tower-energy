...
nonZero_OverlapVsEta = ROOT.TH2D("nonZero_OverlapVsEta","",nBinsEta,minEta,maxEta,30,0,30)
nonZero_OverlapVsEta_fullModules = ROOT.TH2D("nonZero_OverlapVsEta_fullModules","",nBinsEta,minEta,maxEta,30,0,30)
...
for u in range(1+np.max(TCs['waferu'])): #wafer u
    for v in range(1+np.max(TCs['waferv'])): #wafer v
        for l in range(1, 1+int(np.max(TCs['layer'])) ): #layer number
            ...
            wafer_data = TCs[(TCs["waferu"]==u) & (TCs["waferv"]==v) & (TCs["layer"]==l)] 
            if len(wafer_data)!=0:
                counter_total += 1
                ...
                nonZero_OverlapVsEta.Fill(wafer_data["triggercelleta"].mean()*-1.0, nonZero_Overlap_numOfTowers)
                if len(wafer_data)==48:
                    nonZero_OverlapVsEta_fullModules.Fill(wafer_data["triggercelleta"].mean()*-1.0, nonZero_Overlap_numOfTowers)
                
                if ((tower_flat_array[u,v,l][-1 * N_div] == tower_flat_array[u,v,l][-1 * N_div - 1]) and (tower_flat_array[u,v,l][-1 * N_div])!=0):
                    print("--------------------------------------------------")
