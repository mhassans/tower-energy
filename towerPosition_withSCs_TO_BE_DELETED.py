...
SCs = pd.read_csv('cellPositions/sensorCell_positions.txt', sep=' ')
...
#kernel = np.array([[5.e-3, 2.e-2, 5.e-3], [2.e-2, 9.e-1, 2.e-2], [5.e-3, 2.e-2, 5.e-3]])
...
#nonZero_OverlapVsEta_fullsHighDensity = ROOT.TH2D("nonZero_OverlapVsEta_fullsHighDensity","",nBinsEta,minEta,maxEta,45,0,45)
        ...
        for u in range(1+np.max(SCs['waferu'])): #wafer u
            for v in range(1+np.max(SCs['waferv'])): #wafer v
                ...
                if len(wafer_data)!=0:
                    ...
                    #tower_array_gausKernel[u,v,l] = gaussian_filter(tower_array[u,v,l], sigma = 0.2 )
...
print("My program took", (time.time() - start_time)/60.0, "minutes to run")
